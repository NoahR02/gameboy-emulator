...
use crate::{GameBoy, GAMEBOY_CPU_SPEED_WITH_MEMORY_BOTTLE_NECK};
use crate::gui::wgpu_triangle::{render, RenderData};
use crate::ppu::ppu::{ppu_fill_background_tile_map_1, ppu_fill_background_tile_map_2, ppu_fill_oam_map, ppu_fill_tiles};

pub struct GameboyEmulatorState {
...
impl GameboyEmulatorState {
    pub fn new(craft_state: CraftState, game_boy: GameBoy) -> Self {
        Self {
            craft_state,
            render_data: None,
            game_boy,
...
impl ApplicationHandler for GameboyEmulatorState {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        cfg_if::cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                let (renderer, render_data) = craft_state.runtime.borrow_tokio_runtime().block_on(
                    async {
                        let renderer = VelloRenderer::new(window_copy, render_vello_to_texture).await;
                        let render_data = RenderData::new(&renderer);
                        
                        (Box::new(renderer), render_data)
                    }
                );
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _window_id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                let offset = (GAMEBOY_CPU_SPEED_WITH_MEMORY_BOTTLE_NECK / 60) as i64;
                for _ in 0..=offset {
                    self.game_boy.step();
                }

                if self.timer >= 1.0 / self.fixed_fps {
                    println!("fps: {}", fps);
                    ppu_fill_tiles(&mut self.game_boy.bus, &mut self.game_boy.ppu);
                    ppu_fill_background_tile_map_1(&mut self.game_boy.bus, &mut self.game_boy.ppu);
                    ppu_fill_background_tile_map_2(&mut self.game_boy.bus, &mut self.game_boy.ppu);
                    ppu_fill_oam_map(&mut self.game_boy.bus, &mut self.game_boy.ppu);
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if let Some(window) = craft_state.craft_app.window.clone() {
