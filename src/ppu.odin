...
ppu_get_tile_data :: proc(self: ^Ppu, tile_number: u8, force_tile_fetch_method: Tile_Fetch_Mode = .Default_Behavior) -> (tile_data: [8 * 8 * 4]u8) {
    ...
    switch force_tile_fetch_method {
        case .Default_Behavior: {
            lcd_control := transmute(Lcd_Control_Register) memory_mapper_read(self.gb_state.memory_mapper, LCDC)
...
ppu_fill_tile_map_1 :: proc(self: ^Ppu) {
    ...
    for tile_map_index_address in 0x9800..=0x9BFF {
        tile_map_index := memory_mapper_read(self.gb_state.memory_mapper, u16(tile_map_index_address))
