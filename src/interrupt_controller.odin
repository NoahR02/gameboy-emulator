...
interrupt_controller_handle_interrupts :: proc(gb_state: ^Gb_State) {
    interrupts_enabled := transmute(Interrupt_Set)memory_mapper_read(gb_state.memory_mapper, INTERRUPTS_ENABLED)
    ...
    if did_interrupt_occur {
        gb_state.is_halted = false
    }

    if gb_state.cpu.ime {
        using gb_state.memory_mapper
